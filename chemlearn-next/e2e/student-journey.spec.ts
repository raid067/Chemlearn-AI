...
test.describe('ChemLearn Student Journey & Curriculum Flow', () => {
  ...
  test('2. Lessons Catalog: renders Chapter 6 and Chapter 8 KSSM modules', async ({ page }) => {
    ...
    // Assert
    await expect(page.getByText('Acids, Bases and Salts').first()).toBeVisible();
    await expect(page.getByRole('heading', { name: /Manufactured Substances in Industry/i }).first()).toBeVisible();
