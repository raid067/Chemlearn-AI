...
export async function POST(req: NextRequest) {
  try {
    ...
    if (await isRateLimitedAsync('challenge-submit', user.uid, 10, 60_000)) {
      return NextResponse.json({ error: 'Too many challenge submissions. Please wait a moment.' }, { status: 429 });
    }
